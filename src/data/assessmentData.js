module.exports = [
  {
    category: "Communication & Relationships",
    assessment_type: "communication",
    questions: [
      {
        id: "comm-1",
        text: "Do you know how to adapt your communication style for different people (e.g., formal vs casual, professional vs personal)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "comm-1a",
              text: "Can you confidently write a clear professional email or message?",
              parent_id: "comm-1",
            },
            {
              id: "comm-1b",
              text: "Do you know how to switch between formal and casual conversation appropriately?",
              parent_id: "comm-1",
            },
            {
              id: "comm-1c",
              text: "Can you recognise and adapt to non-verbal cues (body language, tone)?",
              parent_id: "comm-1",
            },
            {
              id: "comm-1d",
              text: "Do you know how to adjust your approach when talking to children, elders, or authority figures?",
              parent_id: "comm-1",
            },
          ],
        },
      },
      {
        id: "comm-2",
        text: "How confident are you with conflict resolution in everyday life?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "comm-2a",
              text: "Can you de-escalate a heated argument calmly?",
              parent_id: "comm-2",
            },
            {
              id: "comm-2b",
              text: "Do you know how to negotiate a compromise that works for everyone?",
              parent_id: "comm-2",
            },
            {
              id: "comm-2c",
              text: "Can you stay respectful and constructive when you strongly disagree with someone?",
              parent_id: "comm-2",
            },
            {
              id: "comm-2d",
              text: "Do you know how to support others in resolving disagreements fairly?",
              parent_id: "comm-2",
            },
          ],
        },
      },
      {
        id: "comm-3",
        text: "Do you feel able to express your ideas clearly in group settings (meetings, social events, community)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "comm-3a",
              text: "Can you give a short presentation or speech confidently?",
              parent_id: "comm-3",
            },
            {
              id: "comm-3b",
              text: "Do you know how to keep people engaged while you\u2019re speaking?",
              parent_id: "comm-3",
            },
            {
              id: "comm-3c",
              text: "Can you encourage quieter people to contribute without pressure?",
              parent_id: "comm-3",
            },
            {
              id: "comm-3d",
              text: "Do you know how to chair or facilitate a structured discussion?",
              parent_id: "comm-3",
            },
          ],
        },
      },
      {
        id: "comm-4",
        text: "Do you actively listen and respond thoughtfully when others share their views?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "comm-4a",
              text: "Can you summarise what someone has said to show you\u2019ve understood?",
              parent_id: "comm-4",
            },
            {
              id: "comm-4b",
              text: "Do you know how to ask good follow-up questions to keep conversation flowing?",
              parent_id: "comm-4",
            },
            {
              id: "comm-4c",
              text: "Can you provide constructive feedback without sounding critical?",
              parent_id: "comm-4",
            },
            {
              id: "comm-4d",
              text: "Do you know how to notice when someone feels unheard and bring them back into the conversation?",
              parent_id: "comm-4",
            },
          ],
        },
      },
      {
        id: "comm-5",
        text: "Do you feel able to express your feelings (e.g., sadness, joy, frustration) openly and appropriately?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "comm-5a",
              text: "Can you put words to emotions instead of bottling them up?",
              parent_id: "comm-5",
            },
            {
              id: "comm-5b",
              text: "Do you know how to express vulnerability without feeling ashamed?",
              parent_id: "comm-5",
            },
            {
              id: "comm-5c",
              text: "Can you support someone else who\u2019s sharing their emotions with you?",
              parent_id: "comm-5",
            },
            {
              id: "comm-5d",
              text: "Do you know how to communicate emotions without placing blame?",
              parent_id: "comm-5",
            },
          ],
        },
      },
      {
        id: "comm-6",
        text: "Do you feel comfortable talking openly with other men about personal challenges or worries?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "comm-6a",
              text: "Can you reach out to a friend when you need support?",
              parent_id: "comm-6",
            },
            {
              id: "comm-6b",
              text: "Do you know how to respond if another man confides in you?",
              parent_id: "comm-6",
            },
            {
              id: "comm-6c",
              text: "Can you create space for honest conversations without judgment?",
              parent_id: "comm-6",
            },
            {
              id: "comm-6d",
              text: "Do you know how to balance humour and seriousness when checking in on mates?",
              parent_id: "comm-6",
            },
          ],
        },
      },
    ],
  },
  {
    category: "Community & Contribution",
    assessment_type: "community",
    questions: [
      {
        id: "commu-1",
        text: "How confident are you in taking part in group projects or community initiatives?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "commu-1a",
              text: "Can you take on specific roles (e.g., treasurer, organiser, coordinator)?",
              parent_id: "commu-1",
            },
            {
              id: "commu-1b",
              text: "Do you know how to plan or schedule community activities?",
              parent_id: "commu-1",
            },
            {
              id: "commu-1c",
              text: "Can you manage logistics like booking venues or arranging equipment?",
              parent_id: "commu-1",
            },
          ],
        },
      },
      {
        id: "commu-2",
        text: "How comfortable are you supporting people outside your immediate circle (e.g., neighbours, colleagues, strangers)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "commu-2a",
              text: "Do you offer help proactively without waiting to be asked?",
              parent_id: "commu-2",
            },
            {
              id: "commu-2b",
              text: "Are you able to balance helping others with your own responsibilities?",
              parent_id: "commu-2",
            },
            {
              id: "commu-2c",
              text: "Can you connect people to the right resources or services when needed?",
              parent_id: "commu-2",
            },
          ],
        },
      },
      {
        id: "commu-3",
        text: "How confident are you in sharing your skills with others in a useful way?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "commu-3a",
              text: "Can you explain or demonstrate practical skills step by step?",
              parent_id: "commu-3",
            },
            {
              id: "commu-3b",
              text: "Have you ever created resources (guides, notes, videos) to help others learn?",
              parent_id: "commu-3",
            },
            {
              id: "commu-3c",
              text: "Can you adapt your approach depending on who you are helping?",
              parent_id: "commu-3",
            },
          ],
        },
      },
      {
        id: "commu-4",
        text: "How confident are you in contributing to the wellbeing of your wider community?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "commu-4a",
              text: "Do you participate in keeping shared spaces safe, clean, or welcoming?",
              parent_id: "commu-4",
            },
            {
              id: "commu-4b",
              text: "Have you been part of organising or attending community events that bring people together?",
              parent_id: "commu-4",
            },
            {
              id: "commu-4c",
              text: "Can you identify gaps in your community and suggest improvements?",
              parent_id: "commu-4",
            },
          ],
        },
      },
      {
        id: "init-comm-1",
        text: "How confident are you at expressing your needs in relationships?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-comm-2",
        text: "Are you comfortable giving and receiving feedback?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-comm-3",
        text: "Do you find it easy to have difficult conversations without conflict?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-comm-4",
        text: "Can you maintain long-term friendships or close connections?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-commu-1",
        text: "Are you involved in any community or volunteering activities?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-commu-2",
        text: "Do you feel a sense of responsibility toward others?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-commu-3",
        text: "Have you ever supported someone else in learning or improving a skill?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-commu-4",
        text: "Do you make time to contribute beyond your own household?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
    ],
  },
  {
    category: "DIY & Repairs",
    assessment_type: "diy",
    questions: [
      {
        id: "diy-1",
        text: "What level are you at with everyday gardening tasks (planting, mowing, pruning)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "diy-1a",
              text: "Can you design and maintain a flower bed or vegetable patch?",
              parent_id: "diy-1",
            },
            {
              id: "diy-1b",
              text: "Do you know how to use powered garden tools (hedge trimmer, strimmer)?",
              parent_id: "diy-1",
            },
            {
              id: "diy-1c",
              text: "Can you build or repair garden structures (fence, shed, decking)?",
              parent_id: "diy-1",
            },
            {
              id: "diy-1d",
              text: "Can you install irrigation or drainage systems for a garden?",
              parent_id: "diy-1",
            },
            {
              id: "diy-1e",
              text: "Do you know how to graft or propagate plants successfully?",
              parent_id: "diy-1",
            },
          ],
        },
      },
      {
        id: "diy-2",
        text: "What level are you at with interior improvements (painting, flooring, tiling)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "diy-2a",
              text: "Can you safely fit shelving units or cupboards?",
              parent_id: "diy-2",
            },
            {
              id: "diy-2b",
              text: "Can you install or repair flooring (tiles, laminate, carpet)?",
              parent_id: "diy-2",
            },
            {
              id: "diy-2c",
              text: "Have you ever completed a room renovation project (kitchen, bathroom, loft)?",
              parent_id: "diy-2",
            },
            {
              id: "diy-2d",
              text: "Can you carry out basic plastering or wall repairs?",
              parent_id: "diy-2",
            },
            {
              id: "diy-2e",
              text: "Can you safely remove and replace internal walls or partitions?",
              parent_id: "diy-2",
            },
          ],
        },
      },
      {
        id: "diy-3",
        text: "What level are you at with simple plumbing (fixing leaks, changing taps)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "diy-3a",
              text: "Can you install or replace a toilet?",
              parent_id: "diy-3",
            },
            {
              id: "diy-3b",
              text: "Can you fit or repair a shower or bath?",
              parent_id: "diy-3",
            },
            {
              id: "diy-3c",
              text: "Do you understand basic water pipe layouts (supply vs waste)?",
              parent_id: "diy-3",
            },
            {
              id: "diy-3d",
              text: "Can you install or replace radiators or a heating system component?",
              parent_id: "diy-3",
            },
            {
              id: "diy-3e",
              text: "Can you solder or join copper piping confidently?",
              parent_id: "diy-3",
            },
          ],
        },
      },
      {
        id: "diy-4",
        text: "What level are you at with car maintenance (oil changes, tyres, battery)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "diy-4a",
              text: "Can you replace brake pads or discs?",
              parent_id: "diy-4",
            },
            {
              id: "diy-4b",
              text: "Can you change a timing belt or alternator?",
              parent_id: "diy-4",
            },
            {
              id: "diy-4c",
              text: "Do you understand how to diagnose engine warning lights?",
              parent_id: "diy-4",
            },
            {
              id: "diy-4d",
              text: "Can you service a car fully (oil, filters, spark plugs, coolant, brakes)?",
              parent_id: "diy-4",
            },
            {
              id: "diy-4e",
              text: "Can you replace a clutch or gearbox?",
              parent_id: "diy-4",
            },
          ],
        },
      },
      {
        id: "diy-5",
        text: "What level are you at with common power tools (drill, saw, sander)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "diy-5a",
              text: "Can you safely operate and maintain a circular saw or jigsaw?",
              parent_id: "diy-5",
            },
            {
              id: "diy-5b",
              text: "Have you built furniture or structures using power tools?",
              parent_id: "diy-5",
            },
            {
              id: "diy-5c",
              text: "Do you understand workshop safety (PPE, ventilation, electrics)?",
              parent_id: "diy-5",
            },
            {
              id: "diy-5d",
              text: "Can you build fitted furniture (wardrobes, cabinets, or a workbench)?",
              parent_id: "diy-5",
            },
            {
              id: "diy-5e",
              text: "Can you use advanced tools (router, table saw, lathe) with confidence?",
              parent_id: "diy-5",
            },
          ],
        },
      },
      {
        id: "diy-6",
        text: "Do you own a set of basic tools (e.g., screwdriver set, hammer, pliers)?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "diy-6a",
              text: "Can you sharpen and maintain tools like chisels, saws, or blades?",
              parent_id: "diy-6",
            },
            {
              id: "diy-6b",
              text: "Do you know how to properly oil/grease and store tools to extend their lifespan?",
              parent_id: "diy-6",
            },
            {
              id: "diy-6c",
              text: "Can you identify and select the correct tool for a job without guidance?",
              parent_id: "diy-6",
            },
            {
              id: "diy-6d",
              text: "Do you use safety gear (PPE, eye protection, gloves) consistently when working?",
              parent_id: "diy-6",
            },
            {
              id: "diy-6e",
              text: "Can you measure and mark accurately using tools like spirit levels, squares, or calipers?",
              parent_id: "diy-6",
            },
          ],
        },
      },
      {
        id: "init-diy-1",
        text: "How confident are you using basic tools like a hammer or screwdriver?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-diy-2",
        text: "Can you assemble flat-pack furniture without help?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-diy-3",
        text: "Have you ever repaired something in your home (e.g., a leaking tap, squeaky hinge)?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-diy-4",
        text: "Can you hang a shelf or picture securely on a wall?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
    ],
  },
  {
    category: "Technology & Digital Skills",
    assessment_type: "technology",
    questions: [
      {
        id: "init-tech-1",
        text: "How confident are you setting up new devices (TV, Wi-Fi, smart home tech)?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-tech-2",
        text: "Do you know how to keep your computer or phone software up to date?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-tech-3",
        text: "Can you confidently troubleshoot basic tech issues?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-tech-4",
        text: "Are you comfortable using tools like spreadsheets or word processors?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "tech-1",
        text: "How comfortable are you with using email, browsing, and managing files?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "tech-1a",
              text: "Can you install and configure new software safely?",
              parent_id: "tech-1",
            },
            {
              id: "tech-1b",
              text: "Do you know how to back up files to the cloud or external drives?",
              parent_id: "tech-1",
            },
            {
              id: "tech-1c",
              text: "Can you troubleshoot common issues like a slow computer or error messages?",
              parent_id: "tech-1",
            },
            {
              id: "tech-1d",
              text: "Can you confidently set up a new computer from scratch (updates, accounts, software)?",
              parent_id: "tech-1",
            },
          ],
        },
      },
      {
        id: "tech-2",
        text: "How confident are you with setting up and using a smartphone?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "tech-2a",
              text: "Can you move data/photos from one phone to another?",
              parent_id: "tech-2",
            },
            {
              id: "tech-2b",
              text: "Do you know how to manage storage and clear space?",
              parent_id: "tech-2",
            },
            {
              id: "tech-2c",
              text: "Can you set up parental controls or accessibility features?",
              parent_id: "tech-2",
            },
            {
              id: "tech-2d",
              text: "Do you update apps and the operating system regularly?",
              parent_id: "tech-2",
            },
          ],
        },
      },
      {
        id: "tech-3",
        text: "How confident are you with word processing, spreadsheets, and presentations?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "tech-3a",
              text: "Can you use formulas and charts in spreadsheets?",
              parent_id: "tech-3",
            },
            {
              id: "tech-3b",
              text: "Do you know how to collaborate on shared documents online?",
              parent_id: "tech-3",
            },
            {
              id: "tech-3c",
              text: "Can you make a professional-looking presentation?",
              parent_id: "tech-3",
            },
            {
              id: "tech-3d",
              text: "Do you know how to merge documents or export to PDF?",
              parent_id: "tech-3",
            },
            {
              id: "tech-3e",
              text: "Can you use templates or basic automation to save time?",
              parent_id: "tech-3",
            },
          ],
        },
      },
      {
        id: "tech-4",
        text: "How confident are you using the internet safely and effectively?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "tech-4a",
              text: "Do you know how to shop online safely?",
              parent_id: "tech-4",
            },
            {
              id: "tech-4b",
              text: "Can you spot suspicious websites or scams?",
              parent_id: "tech-4",
            },
            {
              id: "tech-4c",
              text: "Do you use social media with privacy settings enabled?",
              parent_id: "tech-4",
            },
            {
              id: "tech-4d",
              text: "Do you know how to manage cookies and browser settings?",
              parent_id: "tech-4",
            },
            {
              id: "tech-4e",
              text: "Can you set up online banking or government digital services?",
              parent_id: "tech-4",
            },
          ],
        },
      },
      {
        id: "tech-5",
        text: "How confident are you with setting up and managing home internet?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "tech-5a",
              text: "Can you set up or reset a Wi-Fi router?",
              parent_id: "tech-5",
            },
            {
              id: "tech-5b",
              text: "Do you know how to connect devices like printers or smart TVs?",
              parent_id: "tech-5",
            },
            {
              id: "tech-5c",
              text: "Can you fix Wi-Fi problems such as weak signal or devices not connecting?",
              parent_id: "tech-5",
            },
            {
              id: "tech-5d",
              text: "Do you know how to make your Wi-Fi secure with a strong password?",
              parent_id: "tech-5",
            },
            {
              id: "tech-5e",
              text: "Can you set up a guest network for visitors?",
              parent_id: "tech-5",
            },
          ],
        },
      },
      {
        id: "tech-6",
        text: "How confident are you in keeping your accounts and data safe?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "tech-6a",
              text: "Do you use strong and unique passwords?",
              parent_id: "tech-6",
            },
            {
              id: "tech-6b",
              text: "Do you know how to use two-factor authentication?",
              parent_id: "tech-6",
            },
            {
              id: "tech-6c",
              text: "Can you recognise phishing emails or scam texts?",
              parent_id: "tech-6",
            },
            {
              id: "tech-6d",
              text: "Do you update devices and software regularly for security?",
              parent_id: "tech-6",
            },
            {
              id: "tech-6e",
              text: "Do you know how to safely dispose of an old phone or laptop?",
              parent_id: "tech-6",
            },
          ],
        },
      },
    ],
  },
  {
    category: "Well-being & Self-care",
    assessment_type: "self-care",
    questions: [
      {
        id: "init-well-1",
        text: "Do you have a regular exercise or movement routine?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-well-2",
        text: "Do you take time to reflect on your mental well-being?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-well-3",
        text: "Can you confidently prepare a healthy meal?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "init-well-4",
        text: "Do you make time for rest and recovery?",
        parent_id: null,
        followUps: {
          questions: [],
        },
      },
      {
        id: "self-1",
        text: "Do you engage in regular physical activity or exercise?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "self-1a",
              text: "Can you confidently design your own workout routine?",
              parent_id: "self-1",
            },
            {
              id: "self-1b",
              text: "Do you know how to perform key compound exercises with good form?",
              parent_id: "self-1",
            },
            {
              id: "self-1c",
              text: "Can you track and progressively improve your fitness over time?",
              parent_id: "self-1",
            },
            {
              id: "self-1d",
              text: "Do you understand how to warm up and cool down safely?",
              parent_id: "self-1",
            },
          ],
        },
      },
      {
        id: "self-2",
        text: "Do you actively set aside time for mental well-being practices?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "self-2a",
              text: "Do you practice techniques like meditation or journaling?",
              parent_id: "self-2",
            },
            {
              id: "self-2b",
              text: "Can you identify signs of stress or burnout in yourself?",
              parent_id: "self-2",
            },
            {
              id: "self-2c",
              text: "Do you use strategies to manage anxiety or difficult emotions?",
              parent_id: "self-2",
            },
            {
              id: "self-2d",
              text: "Do you maintain healthy digital or work-life boundaries?",
              parent_id: "self-2",
            },
          ],
        },
      },
      {
        id: "self-3",
        text: "Can you prepare balanced and healthy meals for yourself?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "self-3a",
              text: "Do you know how to cook multiple meals from scratch (not ready meals)?",
              parent_id: "self-3",
            },
            {
              id: "self-3b",
              text: "Can you adapt meals to meet dietary needs (e.g., high-protein, vegetarian)?",
              parent_id: "self-3",
            },
            {
              id: "self-3c",
              text: "Do you understand portion sizes and basic nutrition principles?",
              parent_id: "self-3",
            },
            {
              id: "self-3d",
              text: "Can you confidently follow a recipe and adjust it if ingredients are missing?",
              parent_id: "self-3",
            },
          ],
        },
      },
      {
        id: "self-4",
        text: "Do you make consistent time for proper rest and recovery?",
        parent_id: null,
        followUps: {
          questions: [
            {
              id: "self-4a",
              text: "Do you get at least 7\u20138 hours of quality sleep most nights?",
              parent_id: "self-4",
            },
            {
              id: "self-4b",
              text: "Do you avoid screen use or stimulants before bed?",
              parent_id: "self-4",
            },
            {
              id: "self-4c",
              text: "Do you know relaxation techniques to help you wind down?",
              parent_id: "self-4",
            },
            {
              id: "self-4d",
              text: "Do you regularly schedule downtime without feeling guilty?",
              parent_id: "self-4",
            },
          ],
        },
      },
    ],
  },
];
